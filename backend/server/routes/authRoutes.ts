...
export function registerAuthRoutes(router: Router): void {
  ...
  router.get('/auth/user', async (req, res) => {
    try {
      ...
      console.log(`Auth request for email: ${email}`, 'auth');
      ...
      try {
        ...
        if (result.rows.length === 0) {
          console.log(`User not found in database: ${email}`, 'auth');
        ...
        console.log(`User authenticated from database: ${email}`, 'auth');
  ...
  router.post('/auth/login', async (req, res) => {
    try {
      ...
      // Log for debugging
      console.log(`Login attempt for ${username}`, "auth");
      ...
      try {
        ...
        if (result.rows.length === 0) {
          ...
          console.log(`User not found in database: ${username}`, "auth");
        ...
        if (user.password !== password) {
          console.log(`Invalid password for user: ${username}`, "auth");
          return res.status(401).json({ message: "Invalid email or password" });
        }
        
        console.log(`Successful login for database user: ${username}`, "auth");
    ...
    try {
      ...
      try {
        ...
        if (existingUser.length > 0) {
          console.log(`User already exists: ${validated.username}`, "auth");
        ...
        console.log(`New user account created: ${validated.username}`, "auth");
